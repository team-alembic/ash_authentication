...
if Code.ensure_loaded?(Igniter) do
  defmodule Mix.Tasks.AshAuthentication.AddAddOn.AuditLog do
    ...
    Also provisions `:audit_log_ip_salt` under your application: a distinct
    generated value in `dev.exs` and `test.exs`, and a `runtime.exs` entry for
    `:prod` which reads `AUDIT_LOG_IP_SALT` from the environment. The salt is
    only used by `ip_privacy_mode :hash`, and is provisioned so that selecting
    that mode later works without further setup.
