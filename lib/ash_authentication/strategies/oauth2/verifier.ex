...
defmodule AshAuthentication.Strategy.OAuth2.Verifier do
  ...
  @doc """
  Verifies that an OAuth2-derived strategy which registers users is paired with a
  confirmation add-on that monitors the fields its register action upserts on.

  A password strategy which registers users lets an attacker create an account
  carrying the victim's collision value. Without confirmation the victim's first
  sign-in through the provider upserts into that account.

  The confirmation add-on's runtime guard is what blocks that upsert, so this is
  a diagnostic rather than a control. It warns and does not error.
  """
  @spec prevent_hijacking(map, OAuth2.t()) :: :ok | {:warn, [String.t()]}
