...
defmodule AshAuthentication.WebAuthnCredential.Transformer do
  ...
  def transform(dsl_state) do
    with :ok <- validate_wax_dependency(),
      ...
      maybe_build_action(dsl_state, update_action_name, fn dsl_state ->
    ...
    if existing do
      ...
      {:ok, identity} =
        Transformer.build_entity(
          ...
          [:identities],
          :identity,
          [name: identity_name, keys: [credential_id_field]] ++ pre_check_opts(dsl_state)
        )
  ...
  # Data layers with no unique constraint of their own (ETS, Mnesia) have Ash
  # enforce identities with a read before each write, and register
  # `RequirePreCheckWith` to fail any identity that doesn't say which domain
  # to read through. Data layers that enforce identities natively (Postgres,
  # SQLite) don't register it, and get no redundant pre-check.
  #
  # Without a persisted domain there's nothing to infer, so the identity is
  # left as-is and the verifier's own error tells the user what to set.
  defp pre_check_opts(dsl_state) do
    data_layer = Transformer.get_persisted(dsl_state, :data_layer)
    domain = Transformer.get_persisted(dsl_state, :domain)

    if domain && requires_pre_check?(data_layer) do
  ...
  # A pre-checked identity puts a `before_action` hook on every create and
  # update, and an update with hooks outside `after_action` can't run
  # atomically — so a `require_atomic? true` update fails with `MustBeAtomic`.
  # `pre_check?` counts too, since Ash only resolves it to `pre_check_with`
  # in a later transformer.
  defp atomic_opts(dsl_state) do
    pre_checked? =
      dsl_state
      |> Transformer.get_entities([:identities])
      |> Enum.any?(&(&1.pre_check_with || &1.pre_check?))

    if pre_checked?, do: [require_atomic?: false], else: []
  end

  defp requires_pre_check?(nil), do: false

  defp requires_pre_check?(data_layer) do
    Code.ensure_loaded?(data_layer) and function_exported?(data_layer, :verifiers, 0) and
