...
defmodule AshAuthentication.TokenResource.Actions do
  ...
  alias Ash.{Changeset, Error.Changes.InvalidArgument, Error.Changes.StaleRecord, Query, Resource}
  ...
  defp revoke_conditional_upsert(resource, token, opts) do
    with :ok <- assert_resource_has_extension(resource, TokenResource),
      ...
      |> Changeset.for_create(
        ...
        %{"token" => token},
        # A stored token record already carries a trustworthy `expires_at`,
        # written when this library minted the token.  Only flip the purpose, so
  ...
  defp peek_jti(token) do
    case Jwt.peek(token) do
      {:ok, %{"jti" => jti}} when is_binary(jti) and byte_size(jti) > 0 ->
        {:ok, jti}

      _ ->
        {:error, InvalidArgument.exception(field: :token, message: "is not a valid token")}
    end
  end
  ...
  defp revoke_legacy_upsert(resource, token, opts) do
    with :ok <- assert_resource_has_extension(resource, TokenResource),
      ...
      |> Changeset.for_create(
        ...
        # written when this library minted the token.  Only flip the purpose, so
        # that a claim read from the token cannot rewrite that value.
        Keyword.merge(opts, upsert?: true, upsert_fields: [:purpose])
      )
      |> Ash.create(domain: domain)
      |> case do
        {:ok, _} -> :ok
