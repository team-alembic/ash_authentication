...
| [`confirm_setup_enabled?`](#authentication-strategies-totp-confirm_setup_enabled?){: #authentication-strategies-totp-confirm_setup_enabled? } | `boolean` | `false` | When enabled, the setup action will not store the secret directly on the user. Instead, it generates a setup token that must be confirmed with a valid TOTP code. This provides additional security by verifying the user has correctly saved their secret. |
