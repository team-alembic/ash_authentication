...
| [`session_identifier`](#authentication-session_identifier){: #authentication-session_identifier } | `:error \| :jti \| :unsafe` | `:error` | How to uniquely identify a session in the case that `require_token_presence_for_authentication?` is not set to `true`. Should always be `:jti`. Exists for backwards compatibility. |
