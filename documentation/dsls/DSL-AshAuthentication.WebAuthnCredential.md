...
| [`user_id_field`](#webauthn_credential-user_id_field){: #webauthn_credential-user_id_field } | `atom` |  | The name of the foreign key attribute referencing the user (from the belongs_to). Defaults to `nil`, which leaves it to `Ash.Resource.Relationships.BelongsTo`'s own convention (`<user_relationship_name>_id`). Set this only to override that convention with a different column name. |
