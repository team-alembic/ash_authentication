...
| [`brute_force_strategy`](#authentication-strategies-magic_link-brute_force_strategy){: #authentication-strategies-magic_link-brute_force_strategy } | `nil \| {:audit_log, atom} \| {:preparation, module}` |  | How to mitigate brute-force attempts against magic link requests. Set to `{:audit_log, :audit_log_name}` to count recent failures by identity via the audit log add-on, or `{:preparation, MyModule}` to plug in a custom preparation. Defaults to `nil` (no protection). |
| [`audit_log_window`](#authentication-strategies-magic_link-audit_log_window){: #authentication-strategies-magic_link-audit_log_window } | `pos_integer \| {pos_integer, :days \| :hours \| :minutes \| :seconds}` | `{5, :minutes}` | Time window for counting failed attempts when using the `{:audit_log, ...}` brute force strategy. If no unit is provided, then `minutes` is assumed. Defaults to 5 minutes. |
| [`audit_log_max_failures`](#authentication-strategies-magic_link-audit_log_max_failures){: #authentication-strategies-magic_link-audit_log_max_failures } | `pos_integer` | `5` | Maximum allowed failures within the window before blocking when using the `{:audit_log, ...}` brute force strategy. Defaults to 5. |
