...
defmodule AshAuthentication.WebAuthnCredential.VerifierTest do
  ...
  describe "unique_credential_id identity" do
    ...
    # ETS has no unique constraint, so `Ash.DataLayer.Verifiers.RequirePreCheckWith`
    # fails any identity without `pre_check_with` — which Spark reports as a
    # compile warning, and `--warnings-as-errors` as a failed build.
    test "the built identity is pre-checked through the domain on ETS" do
      {module, warnings} = with_compile_warnings(&compile_credential!/0)

      assert %{pre_check_with: AshAuthentication.Test.PermissiveDomain} =
               ResourceInfo.identity(module, :unique_credential_id)

      refute warnings =~ "pre_check_with"

      # The pre-check is a `before_action` hook, so the update can't be atomic.
      assert %{require_atomic?: false} = ResourceInfo.action(module, :update)
    end

    test "the built identity is not pre-checked on Postgres" do
      assert %{pre_check_with: nil} =
               ResourceInfo.identity(Example.WebAuthnCredential, :unique_credential_id)

      assert %{require_atomic?: true} = ResourceInfo.action(Example.WebAuthnCredential, :update)
    end

    test "a user-declared identity is left exactly as declared" do
      {module, warnings} =
        with_compile_warnings(fn ->
          compile_credential_with!(identities: "identity :unique_credential_id, [:credential_id]")
        end)

      assert %{pre_check_with: nil} = ResourceInfo.identity(module, :unique_credential_id)
      assert warnings =~ "Must specify the `pre_check_with` option"
    end
  end

  defp with_compile_warnings(fun) do
    ExUnit.CaptureIO.with_io(:stderr, fun)
  end
