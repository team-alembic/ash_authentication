...
defmodule AshAuthentication.Strategy.OAuth2.ActionsTest do
  ...
  describe "register/2" do
    ...
    test "it matches a returning user by their `(strategy, sub)` identity, not by a provider attribute" do
      {:ok, strategy} = Info.strategy(Example.User, :oauth2)

      user = build_user()
      Ash.Seed.update!(user, %{confirmed_at: DateTime.utc_now()})
      sub = "user:#{user.id}"
      :ok = seed_identity(:oauth2, user, sub)
      ...
      assert {:ok, signed_in_user} =
               Actions.register(
                 ...
                 %{
                   ...
                   "user_info" => %{"nickname" => username(), "uid" => user.id, "sub" => sub},
                   "oauth_tokens" => %{
                     "access_token" => Ecto.UUID.generate(),
                     "expires_in" => 86_400,
                     "refresh_token" => Ecto.UUID.generate()
